    ...
    <div class="space-y-4">
      ...
      <div class="mt-8 flex flex-col space-y-4">
        {#if $isAuthenticated}
          <a
            href="/dashboard"
            class="w-full px-4 py-2 bg-green-500 hover:bg-green-600 text-white rounded text-center"
          >
            대시보드로 이동
          </a>
          
          <button
            class="w-full px-4 py-2 bg-red-500 hover:bg-red-600 text-white rounded"
        ...
        {:else}
          <a
            href="/login"
            class="w-full px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded text-center"
          >
            로그인
          </a>
      ...
      {#if $authStore.user}
        <div class="mt-4 p-4 border rounded dark:border-gray-700">
          ...
          <p class="text-sm dark:text-gray-300">
            {$authStore.user.name || $authStore.user.email}님 환영합니다!
          </p>
        </div>
      {/if}
    </div>
